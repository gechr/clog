...
func TestSpinnerProgressSuccess(t *testing.T) {
	...
	result := Spinner("step 1").
		Str("file", "a.go").
		Progress(context.Background(), func(_ context.Context, callback ProgressCallback) error {
			callback("step 2", Field{Key: "file", Value: "b.go"})
...
func TestSpinnerProgressError(t *testing.T) {
	...
	result := Spinner(
		"loading",
	).Progress(context.Background(), func(_ context.Context, _ ProgressCallback) error {
		return testErr
	})
