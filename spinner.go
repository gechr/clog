...
// ProgressCallback updates the spinner's displayed title and fields during execution.
type ProgressCallback func(title string, fields ...Field)

// Task is a function executed by [SpinnerBuilder.Wait].
type Task func(context.Context) error

// ProgressTask is a function executed by [SpinnerBuilder.Progress].
// The [ProgressCallback] callback allows updating the spinner's title and fields.
type ProgressTask func(context.Context, ProgressCallback) error
...
func (b *SpinnerBuilder) Wait(ctx context.Context, task Task) *WaitResult {
	return b.Progress(ctx, func(ctx context.Context, _ ProgressCallback) error {
		return task(ctx)
	})
}

// Progress executes the task with a spinner whose title and fields
// can be updated via the [ProgressCallback] callback. This is useful for multi-step
...
func (b *SpinnerBuilder) Progress(
	...
	wrapped := func(ctx context.Context) error {
		return task(ctx, func(title string, fields ...Field) {
			titlePtr.Store(&title)
			if len(fields) > 0 {
				merged := mergeFields(b.fields, fields)
				fieldsPtr.Store(&merged)
			}
		})
	}
...
func runSpinner(
	...
	levelPrefix := styles.Levels[InfoLevel].Render(label)
