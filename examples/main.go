...
func main() {
	...
	_ = clog.Spinner("Deploying").
		Str("env", "production").
		Progress(context.Background(), func(_ context.Context, callback clog.ProgressCallback) error {
			callback("Building image")
			time.Sleep(500 * time.Millisecond)
			callback("Pushing image", clog.Field{Key: "tag", Value: "v1.2.3"})
			time.Sleep(500 * time.Millisecond)
			callback("Starting containers")
			time.Sleep(500 * time.Millisecond)
			return nil
		}).
		Prefix("🚀").
	...
	partStyles.Messages[clog.InfoLevel] = italic
	partStyles.Messages[clog.WarnLevel] = italic
	...
	minimalStyles.Messages[clog.ErrorLevel] = lipgloss.NewStyle().Strikethrough(true).Foreground(lipgloss.Color("1"))
	...
	styles.Messages[clog.TraceLevel] = lipgloss.NewStyle().Faint(true).Foreground(lipgloss.Color("6")) // dim cyan
	styles.Messages[clog.DebugLevel] = lipgloss.NewStyle().Foreground(lipgloss.Color("6"))             // cyan
	styles.Messages[clog.InfoLevel] = lipgloss.NewStyle().Foreground(lipgloss.Color("2"))              // green
	styles.Messages[clog.DryLevel] = lipgloss.NewStyle().Foreground(lipgloss.Color("5"))               // magenta
	styles.Messages[clog.WarnLevel] = lipgloss.NewStyle().Foreground(lipgloss.Color("3"))              // yellow
	styles.Messages[clog.ErrorLevel] = lipgloss.NewStyle().Foreground(lipgloss.Color("1"))             // red
	styles.Messages[clog.FatalLevel] = lipgloss.NewStyle().Foreground(lipgloss.Color("1"))             // red
