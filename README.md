...
Recognised `CLOG_LEVEL` values: `trace`, `debug`, `info`, `dry`, `warn`, `warning`, `error`, `fatal`. Setting `trace` or `debug` also enables timestamps.
...
err := clog.Spinner("Processing").
    Progress(ctx, func(ctx context.Context, callback clog.ProgressCallback) error {
        for i, item := range items {
            callback(fmt.Sprintf("Processing (%d/%d)", i+1, len(items)))
...
styles.KeyStyles["status"] = lipgloss.NewStyle().
    Foreground(lipgloss.Color("2"))

// 2. Value styles: exact string matches (sensible defaults included)
//    "true" → green, "false" → red, "<nil>" / "" → grey
styles.ValueStyles["PASS"] = lipgloss.NewStyle().
    Foreground(lipgloss.Color("2"))  // green
styles.ValueStyles["FAIL"] = lipgloss.NewStyle().
...
| --------------- | --------------------------- | --------------------------------------------- |
| `Levels`        | `map[Level]lipgloss.Style`  | Per-level label style                         |
| `Messages`      | `map[Level]lipgloss.Style`  | Per-level message style (unstyled by default) |
| `Key`           | `lipgloss.Style`            | Field key style                               |
...
```go
styles := clog.DefaultStyles()
styles.Messages[clog.ErrorLevel] = lipgloss.NewStyle().
    Foreground(lipgloss.Color("1")) // red
styles.Messages[clog.WarnLevel] = lipgloss.NewStyle().
