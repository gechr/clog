...
func TestHumanBytes(t *testing.T) {
	...
	for _, tt := range tests {
		assert.Equal(t, tt.out, humanBytes(tt.in, tt.digits),
			"humanBytes(%d, %d)", tt.in, tt.digits)
	}
}
...
func TestHumanIBytes(t *testing.T) {
	...
	for _, tt := range tests {
		assert.Equal(t, tt.out, humanIBytes(tt.in, tt.digits),
			"humanIBytes(%d, %d)", tt.in, tt.digits)
	}
}

func TestHumanBytesWidth(t *testing.T) {
	// Width must be >= any stripped value in the same unit range.
	w := humanBytesWidth(uint64(100*mByte), 3) // total = 100 MB
	assert.GreaterOrEqual(t, w, len(humanBytes(82854982, 3)))
	assert.GreaterOrEqual(t, w, len(humanBytes(uint64(100*mByte), 3)))
	assert.GreaterOrEqual(t, w, len(humanBytes(uint64(50*mByte), 3)))
}
